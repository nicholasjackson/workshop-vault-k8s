...
```bash
PGPASSWORD=$DB_PASS psql -h 10.5.0.201 -U $DB_USER -d demo -c "SELECT current_user;"
```
...
```bash
PGPASSWORD=postgres psql -h 10.5.0.201 -U postgres -d demo -c "\du"
```
...
```bash
PGPASSWORD=$DB_PASS psql -h 10.5.0.201 -U $DB_USER -d demo -c "\du"
```
