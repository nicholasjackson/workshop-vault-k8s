...
```
postgresql://{{username}}:{{password}}@10.10.0.201:5432/demo?sslmode=disable
```
...
```bash
psql -h 10.10.0.201 -U postgres -d demo -c "\du"
```
...
```bash
psql: error: connection to server at "10.10.0.201", port 5432 failed: FATAL:  password authentication failed for user "postgres"
```
